...
async def actionRescue(getEnv, get):
    ...
    # str(beliefs.get('accident'))+". Time:"+str(time.time()))
...
goal1 = Goal(
    desc="Rescue victim",
    promotions=[
        GoalPromotion(f=goalPromotionRescueToExecutive, desc="Promoting serious accidents",
                      name="executive")
    ],
    plans=[
        Plan(
            ...
            priority=0,
            actions=[
                Action(f=actionRescue, desc="Rescue victim")
            ]
        )
    ]
)
...
goal2 = Goal(
    desc="Recharge battery",
    promotions=[
        GoalPromotion(f=goalRechargeBatteryToExecutive, desc="Promote recharge battery",
                      name="executive")
    ],
    plans=[
        Plan(
...
agent = Agent(
    ...
    conflicts=[
        Conflict(goals=[goal1, goal2],
                 desc="Charge battery instead of rescuing victim")
    ]
)
